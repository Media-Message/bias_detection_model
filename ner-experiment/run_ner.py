...
from trainer import ClassBalancedTrainer
...
def main():
    ...
    wandb.init(project='mm-ner-project-{}'.format(data_args.dataset_name))
    ...
    if data_args.dataset_name in ['sample', 'full-cased', 'full-uncased', 'all-cased']:
        datasets = load_dataset('dataset_loader.py', data_args.dataset_name)
    ...
    label_column_name = 'class_labels'

    # TODO: hack to get biased examples to address class imbalance...
    datasets = datasets.datasets.filter(lambda x: 1 in x['class_labels'])
    ...
    config = AutoConfig.from_pretrained(
        ...
        id2label=id2label_map,  # new
        label2id=label2id,  # new
    ...
    if training_args.do_train:
        ...
        if trainer.is_world_process_zero():
            ...
            # Need to save the state, since Trainer.save_model saves
            # only the tokenizer with the model
